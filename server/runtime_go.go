...
// @group config
// @summary Get a read-only subset of the Nakama server configuration.
// @return config(runtime.Config) A read-only configuration object.
...
// @group events
// @summary Register a function that handles custom events received or generated by the server.
// @param fn(type=function) The function to execute when a custom event is received.
// @return error(error) An optional error value if an error occurred.
...
// @group events
// @summary Register a function that is triggered when client sessions start.
// @param fn(type=function) The function to execute when a client session starts.
// @return error(error) An optional error value if an error occurred.
...
// @group events
// @summary Register a function that is triggered when client sessions end.
// @param fn(type=function) The function to execute when a client session ends.
// @return error(error) An optional error value if an error occurred.
...
func (ri *RuntimeGoInitializer) RegisterRpc(id string, fn func(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, payload string) (string, error)) error {
	...
	ri.rpc[id] = func(ctx context.Context, headers, queryParams map[string][]string, traceID, userID, username string, vars map[string]string, expiry int64, sessionID, clientIP, clientPort, lang, payload string) (string, error, codes.Code) {
		ctx = NewRuntimeGoContext(ctx, ri.node, ri.version, ri.env, RuntimeExecutionModeRPC, headers, queryParams, traceID, expiry, userID, username, vars, sessionID, clientIP, clientPort, lang)
		result, fnErr := fn(ctx, RuntimeLoggerWithTraceId(ctx, ri.logger.WithField("rpc_id", id)), ri.db, ri.nk, payload)
		if fnErr != nil {
...
func (ri *RuntimeGoInitializer) RegisterBeforeRt(id string, fn func(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, envelope *rtapi.Envelope) (*rtapi.Envelope, error)) error {
	...
	ri.beforeRt[id] = func(ctx context.Context, logger *zap.Logger, traceID, userID, username string, vars map[string]string, expiry int64, sessionID, clientIP, clientPort, lang string, envelope *rtapi.Envelope) (*rtapi.Envelope, error) {
		ctx = NewRuntimeGoContext(ctx, ri.node, ri.version, ri.env, RuntimeExecutionModeBefore, nil, nil, traceID, expiry, userID, username, vars, sessionID, clientIP, clientPort, lang)
		loggerFields := map[string]interface{}{"api_id": apiID, "mode": RuntimeExecutionModeBefore.String()}
		return fn(ctx, RuntimeLoggerWithTraceId(ctx, ri.logger.WithFields(loggerFields)), ri.db, ri.nk, envelope)
	}
	return nil
}
...
func (ri *RuntimeGoInitializer) RegisterAfterRt(id string, fn func(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, out, in *rtapi.Envelope) error) error {
	apiID := strings.ToLower(id)
	id = strings.ToLower(RTAPI_PREFIX) + apiID
	ri.afterRt[id] = func(ctx context.Context, logger *zap.Logger, traceID, userID, username string, vars map[string]string, expiry int64, sessionID, clientIP, clientPort, lang string, out, in *rtapi.Envelope) error {
		ctx = NewRuntimeGoContext(ctx, ri.node, ri.version, ri.env, RuntimeExecutionModeAfter, nil, nil, traceID, expiry, userID, username, vars, sessionID, clientIP, clientPort, lang)
...
func (ri *RuntimeGoInitializer) RegisterBeforeGetAccount(fn func(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule) error) error {
	ri.beforeReq.beforeGetAccountFunction = func(ctx context.Context, logger *zap.Logger, traceID, userID, username string, vars map[string]string, expiry int64, clientIP, clientPort string) (error, codes.Code) {
		...
		fnErr := fn(ctx, RuntimeLoggerWithTraceId(ctx, ri.logger.WithFields(loggerFields)), ri.db, ri.nk)
		if fnErr != nil {
			var runtimeErr *runtime.Error
			if errors.As(fnErr, &runtimeErr) {
				if runtimeErr.Code <= 0 || runtimeErr.Code >= 17 {
...
func (ri *RuntimeGoInitializer) RegisterAfterGetAccount(fn func(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, out *api.Account) error) error {
	ri.afterReq.afterGetAccountFunction = func(ctx context.Context, logger *zap.Logger, traceID, userID, username string, vars map[string]string, expiry int64, clientIP, clientPort string, out *api.Account) error {
		...
		return fn(ctx, RuntimeLoggerWithTraceId(ctx, ri.logger.WithFields(loggerFields)), ri.db, ri.nk, out)
	}
	return nil
}
...
func (ri *RuntimeGoInitializer) RegisterAfterUpdateAccount(fn func(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, in *api.UpdateAccountRequest) error) error {
	ri.afterReq.afterUpdateAccountFunction = func(ctx context.Context, logger *zap.Logger, traceID, userID, username string, vars map[string]string, expiry int64, clientIP, clientPort string, in *api.UpdateAccountRequest) error {
		...
		return fn(ctx, RuntimeLoggerWithTraceId(ctx, RuntimeLoggerWithTraceId(ctx, ri.logger.WithFields(loggerFields))), ri.db, ri.nk, in)
	}
	return nil
}
...
func (ri *RuntimeGoInitializer) RegisterAfterDeleteAccount(fn func(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule) error) error {
	ri.afterReq.afterDeleteAccountFunction = func(ctx context.Context, logger *zap.Logger, traceID, userID, username string, vars map[string]string, expiry int64, clientIP, clientPort string) error {
		...
		return fn(ctx, RuntimeLoggerWithTraceId(ctx, ri.logger.WithFields(loggerFields)), ri.db, ri.nk)
	}
	return nil
}
...
// @group matchmaker
// @summary Register a function that is called when matchmaker entries are matched together. The function should return a match ID for a new or existing match to place the matched entries into. Return an empty string to use the default match creation behavior.
// @param fn(type=function) The function to execute when matchmaker entries are matched.
// @return error(error) An optional error value if an error occurred.
...
// @group matchmaker
// @summary Register a function to override the default matchmaker candidate groupings. Receives candidate match groups and returns modified groupings.
// @param fn(type=function) The function to execute to override matchmaker candidate groupings.
// @return error(error) An optional error value if an error occurred.
...
// @group tournaments
// @summary Register a function that is called when a tournament ends.
// @param fn(type=function) The function to execute when a tournament ends.
// @return error(error) An optional error value if an error occurred.
...
// @group tournaments
// @summary Register a function that is called when a tournament resets.
// @param fn(type=function) The function to execute when a tournament resets.
// @return error(error) An optional error value if an error occurred.
...
// @group leaderboards
// @summary Register a function that is called when a leaderboard resets.
// @param fn(type=function) The function to execute when a leaderboard resets.
// @return error(error) An optional error value if an error occurred.
...
// @group purchases
// @summary Register a function to handle Apple App Store purchase notifications.
// @param fn(type=function) The function to execute when an Apple purchase notification is received.
// @return error(error) An optional error value if an error occurred.
...
// @group subscriptions
// @summary Register a function to handle Apple App Store subscription notifications.
// @param fn(type=function) The function to execute when an Apple subscription notification is received.
// @return error(error) An optional error value if an error occurred.
...
// @group purchases
// @summary Register a function to handle Google Play purchase notifications.
// @param fn(type=function) The function to execute when a Google Play purchase notification is received.
// @return error(error) An optional error value if an error occurred.
...
// @group subscriptions
// @summary Register a function to handle Google Play subscription notifications.
// @param fn(type=function) The function to execute when a Google Play subscription notification is received.
// @return error(error) An optional error value if an error occurred.
...
// @group storage
// @summary Create a new storage index and trigger an indexing process for existing data if needed.
// @param name(type=string) The unique identifier of the index.
// @param collection(type=string) The storage collection to index.
// @param key(type=string) The specific storage key to index. Use an empty string to index all keys in the collection.
// @param fields(type=[]string) The document fields to index.
// @param sortableFields(type=[]string) The fields that can be used for sorting query results.
// @param maxEntries(type=int) The maximum number of entries to keep in the index.
// @param indexOnly(type=bool) If true, objects are indexed but not persisted to storage.
// @return error(error) An optional error value if an error occurred.
...
// @group fleetmanager
// @summary Register a FleetManager implementation. The fleet manager can be retrieved later from the runtime using GetFleetManager().
...
// @group shutdown
// @summary Register a function that is executed when the server receives a termination signal. This function only fires if shutdown_grace_sec is greater than 0, and is terminated early if execution takes longer than the configured grace period.
// @param fn(type=function) The function to execute on server shutdown.
// @return error(error) An optional error value if an error occurred.
...
// @group http
// @summary Attach a new HTTP handler to a specified path on the client API server endpoint.
// @param pathPattern(type=string) The URL path pattern to attach the handler to.
// @param handler(type=function) The HTTP handler function to execute for matching requests.
// @param methods(type=string, optional=true) The HTTP methods to accept. If not specified, all methods are accepted.
// @return error(error) An optional error value if an error occurred.
...
// @group http
// @summary Attach a new HTTP handler to a specified path on the console API server endpoint.
// @param pathPattern(type=string) The URL path pattern to attach the handler to.
// @param handler(type=function) The HTTP handler function to execute for matching requests.
// @param methods(type=string, optional=true) The HTTP methods to accept. If not specified, all methods are accepted.
// @return error(error) An optional error value if an error occurred.
...
// @group matches
// @summary Register an authoritative match handler with the given name. The name is used to create new match instances using the NakamaModule MatchCreate function.
// @param name(type=string) The unique identifier of the match handler.
// @param fn(type=function) The function to execute to create a new match instance. Must return a Match implementation.
// @return error(error) An optional error value if an error occurred.
